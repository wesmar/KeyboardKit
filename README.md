...
ComponentTypePrivilege LevelPrimary Functionkvckbd.sysKernel Filter DriverRing 0IRP-level keyboard interception, scan code processing, WSK networkingExplorerFrame.dllCOM In-Process ServerRing 3 (User)CLSID hijacking for persistence, API hooking for watermark removalUdpLogger.exeService ManagerRing 3 (TI/Admin)Installation orchestration, UDP logging, system status monitoring
...
Structurex86 Offsetx64 OffsetPurposeIO_REMOVE_LOCK0x100x20Prevents device removal during operationsKSPIN_LOCK0x6C0xA0Synchronizes access to IRP queueLIST_ENTRY0x700xA8Head of pending read IRP queue
...
International Support: Polish diacritics via AltGr combinations (ą, ć, ę, ł, ń, ó, ś, ź, ż)
...
Start WSK subsystem (WSKStartup)
Resolve target address via WSKGetAddrInfo (supports both IPv4/IPv6)
Create datagram socket (WSKSocket with SOCK_DGRAM)
Configure default destination via SIO_WSK_SET_SENDTO_ADDRESS ioctl
...
Signal shutdown: Set IsUnloading flag
Unhook devices: Restore original DeviceObject pointers
Wait for safety: Poll SafeUnload flag on each keyboard object
Flush buffers: Send any remaining buffered keystrokes
Release resources: Close sockets, delete resources, free pool memory
Delete device: Remove filter device object
...
Locate IAT: Parse PE headers of shell32.dll in memory
Find import entries: Scan import directory for LoadStringW and ExtTextOutW
Modify function pointers: Replace original addresses with proxy functions
VirtualProtect: Temporarily disable page protection to write
...
Installer - Deploys and configures all components with TrustedInstaller privileges
Service - Runs as Windows Service to receive and log UDP keystroke data
Manager - Provides CLI for controlling driver and service states
Monitor - Comprehensive system status checking and diagnostics
...
cppstatic constexpr std::array<BYTE, 7> XOR_KEY = { 
    0xA0, 0xE2, 0x80, 0x8B, 0xE2, 0x80, 0x8C 
};
Extraction Process:

Load executable's own resources (RT_RCDATA)
Parse ICO header to determine image size
Skip ICO data, read remaining bytes as payload
XOR-decrypt payload
Parse TLV structure
Extract individual files (kvckbd.sys, ExpIorerFrame.dll)
Validate PE signatures
...
Writes footer to current file (session duration summary)
Closes file handle
Opens new file with today's date
Writes header to new file
...
ElementNameTypePurpose23000003CurrentBootEntryPointerGUIDPoints to active boot entry16000049TestSigningEnabledBINARYEnables test-signed drivers12000004DescriptionSTRINGOS description text11000001DeviceBINARYBoot device information26000010BootDebuggerEnabledBOOLEANKernel debugger status
...
Driver information is registered in the registry
SCM reads registry at boot
SCM loads drivers based on Start type
Drivers execute from their registered ImagePath
...
Required Values:
Value NameTypeDataPurposeTypeREG_DWORD0x00000001SERVICE_KERNEL_DRIVERStartREG_DWORD0x00000002SERVICE_AUTO_START (loads at boot)ErrorControlREG_DWORD0x00000001SERVICE_ERROR_NORMAL (log error, continue boot)ImagePathREG_EXPAND_SZ\System32\...\kvckbd.sysRelative path to driverDisplayNameREG_SZ"kvckbd"Human-readable name
Value Explanations:
Type Values:
...
cppstd::wstring GetDriverStorePath() {
...
Locate DriverStore keyboard directory
Copy kvckbd.sys to this directory (requires TI privileges)
Construct relative ImagePath for registry
...
cppSC_HANDLE hService = CreateServiceW(
...
cpp// Using TrustedInstaller token (already acquired)
...
Driver service registered in registry
System reboots
Test signing enabled (BCD modified)
SCM loads kvckbd.sys automatically
Driver starts intercepting keyboard

Verification Commands
Check Service Registration:
cmdsc query kvckbd
Check Registry:
cmdreg query HKLM\SYSTEM\CurrentControlSet\Services\kvckbd
Check Driver in Memory:
cmdsc query type=driver state=all | findstr /i kvckbd
Or using DriverView tools:
cmddriverquery /v | findstr /i kvckbd
...
Enter BIOS/UEFI setup (usually F2, Del, F10 during boot)
...
Automatic Reboot:
cmdUdpLogger.exe install --reboot
...
Service Status:
cmdUdpLogger.exe service status
Driver Status:
cmdUdpLogger.exe driver status
Service Management
Start Service:
cmdUdpLogger.exe service start
Stop Service:
cmdUdpLogger.exe service stop
Restart Service:
cmdUdpLogger.exe service restart
...
Restart Driver:
cmdUdpLogger.exe driver restart
...
%TEMP%\keyboard_log_YYYY-MM-DD.txt
%TMP%\keyboard_log_YYYY-MM-DD.txt
%USERPROFILE%\Documents\keyboard_log_YYYY-MM-DD.txt
Current directory

Viewing Logs:
cmdtype %TEMP%\keyboard_log_2025-10-10.txt
...
cmdbcdedit /deletevalue testsigning

Technical Details & Internals
Synchronization Mechanisms
IRQL-Aware Design:
The driver operates at multiple IRQL (Interrupt Request Level) contexts:
ContextIRQLAllowed OperationsSystem threadPASSIVE_LEVELAll operations, can waitIRP dispatch≤ DISPATCH_LEVELNo paged memory, no blockingSpinlock heldDISPATCH_LEVELMinimal operations only
Synchronization Primitives Used:

KSPIN_LOCK (Line Buffer):
...
LIST_ENTRY (Keyboard Object List):
...
c#define KBDDRIVER_POOL_TAG 'dbKK'
Benefits:
...
Example:
cPKBDDRIVER_KEYBOARD_OBJECT obj = ExAllocatePool2(
    POOL_FLAG_NON_PAGED,
...
cVOID KbdIrp_Cancel(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
...
cPDRIVER_CANCEL oldRoutine = IoSetCancelRoutine(irp, NULL);
if (oldRoutine) {
    // Safe to use IRP
} else {
...
ExplorerFrame.dll (Release | x64)
kvckbd.sys (Release | x64)
Resource packaging script (embed DLL and SYS in ICO)
UdpLogger.exe (Release | x64)

Configuration:
All projects use Release configuration with aggressive optimizations.
Driver Signing
Test Signing (Development):
cmdmakecert -r -pe -ss PrivateCertStore -n "CN=TestDriverCert" TestDriverCert.cer
...
powershell$f='$(OutDir)$(TargetName)$(TargetExt)'
...
Network Capture:
cmd# Capture localhost UDP with Wireshark
# Set filter: udp.port == 31415
Registry Monitoring:
cmd# Use Process Monitor (procmon.exe)
...
cmd   sc config kvckbd start= disabled

Delete driver service:

cmd   sc delete kvckbd
